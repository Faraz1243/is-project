...
from Card import read_smartcard
...
class MyWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.url = QtCore.QUrl.fromLocalFile("./sound.mp3")
    ...
    def retranslateUi(self):
        ...
        if not os.path.exists(appdataSavefilePath):
            ...
            tempPasscode = tempPasscode.split("\n")
            self.thePasscode = tempPasscode[0]
            self.atr = tempPasscode[1]
    ...
    def equalButtonClicked(self):
        ...
        if self.settingPasscode == True:
            if self.isFirstPasscode == True:
                ...
                if Card == "atr not found":
                    # show text message
                    font = QtGui.QFont()
                    font.setPointSize(18)
                    self.label.setFont(font)
                    self.label.setText("Please insert your smart card")
                    self.showMessage()

                elif self.firstPasscode == self.secondPasscode:
                    saveFile = open(appdataSavefilePath, "w")
                      # Write the passcode to file
            ...
            if Card == self.atr:
              self.theFileEncryptionWidget.show()
                ...
                font.setPointSize(13)
                self.label.setFont(font)
                self.label.setText("Smart key not matched")
        elif result == 5555:
...
if __name__ == "__main__":
    ...
    appdataSavefilePath = "D:\keys" + "\Save"
